...
class SleepMultiChannelNet(nn.Module):
    def __init__(self, lstm_option):
        ...
        self.CNNBlocks['eog'] = CNNBlock(1)
        self.CNNBlocks['emg'] = CNNBlock(1)
        self.linear = nn.Linear(2880,5)
        self.init_weights()
    def init_weights(self):
        self.linear.weight.data.normal_(0, 0.05)
    def forward(self, x):
        cnn_eeg = self.CNNBlocks['eeg'](x[:,:,0:2,:].view(-1,2,time_period_sample))
        cnn_eog = self.CNNBlocks['eog'](x[:,:,2:3,:].view(-1,1,time_period_sample))
        cnn_emg = self.CNNBlocks['emg'](x[:,:,3:4,:].view(-1,1,time_period_sample))
